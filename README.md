...
* **Our Generalization:** We formalize our latent consistency loss ( $$\mathcal{L}_{lin}$$ ) as the exact continuous-time counterpart of this theory. Because we learn a single continuous generator $$\mathbf{K}_{\text{cont}}$$, we enforce forward-backward trajectory consistency by integrating the dynamics at $\Delta t$ and $-\Delta t$. This mathematically guarantees $e^{\mathbf{K}\Delta t} e^{-\mathbf{K}\Delta t} = I$ without requiring separate matrices.
